...
describe("zvec platform support", () => {
  it("recognizes supported targets", () => {
    ...
    expect(support.supportedTargets).toEqual(["darwin/arm64", "linux/arm64"]);
    expect(isZvecPlatformSupported("linux", "arm64")).toBe(true);
  });

  it("formats unsupported targets with the supported matrix", () => {
    expect(isZvecPlatformSupported("linux", "x64")).toBe(false);
    expect(isZvecPlatformSupported("win32", "x64")).toBe(false);
    expect(formatZvecPlatformSupport("win32", "x64")).toBe(
      "win32/x64 unsupported (supported: darwin/arm64, linux/arm64)",
    );
