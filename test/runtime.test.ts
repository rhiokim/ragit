...
describe("runtime support contract", () => {
  ...
  it("accepts only the production-supported zvec target matrix", () => {
    ...
    expect(getZvecPlatformSupport("linux", "x64").supported).toBe(true);
  ...
  it("combines Node and native target support into one diagnostic contract", () => {
    expect(getRagitRuntimeSupport("24.0.0", "linux", "x64")).toMatchObject({ supported: true });
    expect(getRagitRuntimeSupport("22.13.1", "linux", "x64")).toMatchObject({ supported: false });
    expect(getRagitRuntimeSupport("24.0.0", "win32", "x64")).toMatchObject({ supported: false });

    expect(() => assertRagitRuntime("22.13.1", "linux", "x64")).toThrow(
