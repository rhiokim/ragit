...
const formatStatusText = (status: Awaited<ReturnType<typeof runStatus>>): string =>
  [
    ...
    `- node_runtime: ${status.runtime.node.current}`,
    `- node_minimum: >=${status.runtime.node.minimum}`,
    `- runtime_target: ${status.runtime.platform.current}`,
    `- runtime_supported: ${status.runtime.supported}`,
    `- runtime_supported_targets: ${status.runtime.platform.supportedTargets.join(", ")}`,
