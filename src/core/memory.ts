...
export const formatRecallPacket = (packet: RecallPacket, view: CliView = "default"): { markdown: string; json: string } => {
  ...
  const markdown = [
    ...
    `- snapshot_status: ${projected.snapshot.status}`,
    `- snapshot_sha: ${projected.snapshotSha ?? "none"}`,
    `- branch: ${projected.snapshot.branch ?? "none"}`,
    `- detached: ${projected.snapshot.detached}`,
    `- worktree_dirty: ${projected.snapshot.worktreeDirty}`,
