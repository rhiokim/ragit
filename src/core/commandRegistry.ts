...
const COMMAND_SPECS: CommandDescribeSpec[] = [
  {
    ...
    outputSchemaSummary: [
      ...
      "bootstrap.gitignore.plannedEntries[]",
      "bootstrap.gitignore.addedEntries[]",
      "nextActions[]",
    ],
    arguments: [],
    options: [
  ...
  },
  {
    ...
    outputSchemaSummary: [
      ...
      "summary.validationCounts.verified",
      "summary.validationCounts.attention",
      "summary.validationCounts.unverified",
      "warnings[]",
    ],
    arguments: [{ name: "revRange", type: "string", required: false, description: "선택적인 git revision range" }],
    options: [
  ...
  },
  {
    ...
    outputSchemaSummary: [
      ...
      "selectedHits",
      "selection.strategy",
      "selection.candidateHits",
      "selection.uniqueCitations",
      "selection.selectedSources",
      "selection.duplicateCitationsSkipped",
      "selection.budgetRejectedHits",
      "hits[]",
      "redactionSummary",
    ],
    ...
    options: [
      { name: "--input", type: "path", description: "JSON 입력 파일 경로 또는 -" },
      { name: "--budget", type: "number", description: "공백 구분 콘텐츠 단위 기준의 양의 안전한 정수 예산", defaultValue: 1200 },
      { name: "--scope", type: "enum", description: "검색 범위", enum: ["durable", "session", "harness", "evidence", "all"], defaultValue: "durable" },
      { name: "--at", type: "string", description: "조회할 snapshot SHA 또는 prefix" },
      { name: "--view", type: "enum", description: "출력 축소 수준", enum: ["minimal", "default", "full"], defaultValue: "default" },
      { name: "--format", type: "enum", description: "출력 형식", enum: ["text", "json", "both"], defaultValue: "both" },
