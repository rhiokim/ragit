...
- Provider credentials are still env-only. An OpenAI apply requires `OPENAI_API_KEY` to already be present in the command environment; there is no silent fallback from a requested production provider to `local-placeholder`.
