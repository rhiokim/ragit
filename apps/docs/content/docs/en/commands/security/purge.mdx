...
`security purge` is the remediation companion to `security audit`. It either plans or applies cleanup against local RAGit-owned state such as control-plane files, the searchable store, embedding cache, and quarantine ledgers.
