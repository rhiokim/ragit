...
- A requested production provider never silently falls back to `local-placeholder`. Inspect `status`, then follow the migration preview/apply flow after changing provider or model.
