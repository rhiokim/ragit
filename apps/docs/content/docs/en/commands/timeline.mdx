...
`timeline` reads the append-only event ledger under `.ragit/log/events/*.jsonl` and projects the operational time axis for session materialization, artifact review, memory workflows, harness workflows, and ingest completion. It intentionally does not summarize snapshot semantic state; that is `log`'s job.
...
- If you need artifact-backed beliefs, open loops, and evidence for a commit, use <a href="/en/docs/commands/log">`log`</a> instead of `timeline`.
