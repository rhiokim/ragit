---
title: log
description: raw commit history가 아니라 snapshot 중심 semantic history를 읽습니다
command_path: log
---

## 무엇을 하는 명령인가

`log`는 commit 순서에 맞춰 snapshot history를 읽고, 커밋 사이에서 인덱싱된 지식이 의미적으로 어떻게 달라졌는지 요약합니다. 기본 단위는 `git log`처럼 "어떤 파일이 바뀌었는가?"가 아니라 "다음 agent가 써야 할 searchable knowledge가 어떻게 달라졌는가?"입니다.
...
- JSON 출력에는 `revRange`, `maxCount`, `showMissing`, `filters`, `entries`가 포함됩니다.
